...
class MeiyaParser():
    ...
    def set_home_directory(self, home_directory):
        if home_directory.endswith("Data"):
            ...
            self.home_directory = home_directory.replace("data", "")
        else: self.home_directory = home_directory
    ...
    def save_csv_by_tree_node(self, row_list, tree):
        try:
            ...
            csv_file_path = os.path.join(self.out_put_path,
                                         hm.hexdigest()) + ".csv"
            ...
            with open(csv_file_path, "a+", newline='',
                      encoding='utf-8') as csv_file:
                if os.path.getsize(csv_file_path) > 4:
                    row_list.pop(0)
                csv_writer = csv.writer(csv_file,
                                        delimiter='\t',
                                        quoting=csv.QUOTE_ALL)
                ...
                for row in row_list:
                    try:
                        ...
                        csv_writer.writerow([
                            str(pattern2.sub('',
                                             element).encode(encoding='utf-8',
                                                             errors='strict'),
                                encoding='utf-8') for element in row
                        ])
                        #new_row_list.append()
                    except Exception as e:
                        pass
                ...
                csv_file.close()
                if [hm.hexdigest() + ".csv",
                        parent_tree + ".csv"] not in self.md5name:
                    self.md5name.append(
                        [hm.hexdigest() + ".csv", parent_tree + ".csv"])
        except Exception as e:
            logging.getLogger(__name__).exception(e)
            pass
